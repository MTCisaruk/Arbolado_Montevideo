...
```{r preprocesamiento y datos auxiliares, eval = FALSE}
...
x_train <- fideos %>% filter(!is.na(precio)) %>% select(year, month) %>% as.matrix() %>% xgboost::xgb.DMatrix()

y_train <- c(fideos %>% filter(!is.na(precio)) %>% select(precio))[[1]]

x_pred <- fideos %>% filter(is.na(precio)) %>% select(year, month) %>% as.matrix() %>% xgboost::xgb.DMatrix()
