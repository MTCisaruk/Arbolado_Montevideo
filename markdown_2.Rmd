...
A lo largo del informe se usarán los datos de los precios de los fideos como un proxy para explicar los datos, dado que no se encontró una forma de ponderar los precios de la canasta básica. Sin embargo, la aplicación de Shiny permitirá seleccionar los productos de manera individual.
...
Para comenzar con el análisis de los datos, se intentó visualizar a grandes rasgos las características de los productos y establecimientos disponibles.

Para los establecimientos, resulta interesante ver cómo se distribuyen en el territorio del país. Para eso se graficó un mapa donde cada uno es representado por una cruz. Además, a través de un gráfico de barras se presenta la cantidad por departamento.
...
```{r fig.cap = "Cantidad de establecimientos por departamento. Mas del 60% se concentran en Montevideo"}
...
Como es posible ver en ambos gráficos, los establecimientos encuestados se distribuyen principalmente sobre la costa y en la capital. Algunos departamentos casi no están representados, dada la baja cantidad de establecimientos encuestados.
...
Cómo se puede observar, la mayoría de los establecimientos forman parte de una cadena. A continuación se presentan las diez cadenas con la mayor cantidad de establecimientos
...
Este gráfico permite dar cuenta que más de la mitad de los establecimientos relevados corresponden a solamente cinco cadenas. Sumando las diez con mayor cantidad, ese porcentaje alcanza el 70%.


Por otro lado, como referencia de los productos se hizo una comparativa de los precios de fideos a lo largo del tiempo entre las distintas marcas y tipos (de sémola o al huevo). Se observó que todos los productos aumentan a un mismo ritmo con excepción de los fideos Puritas, que mantuvieron su precio durante años, lo cual resulta bastante extraño.

```{r, fig.cap="Precio de los fideos a lo largo del tiempo"}
precios %>% 
...
Analizando los precios de los fideos por región se notó bastante homogeneidad a nivel departamental, con algunas excepciones que también se podrían explicar por el bajo tamaño de muestra o la ausencia de marcas baratas/caras en alguno de los departamentos.

Dentro de Montevideo se notaron más diferencias. Los barrios de mayor poder adquisitivo tienen los precios más elevados y las áreas rurales los precios más bajos.
...
En cuanto a las herramientas utilizadas, se exploraron varias alternativas hasta llegar a una satisfactoria. En primer lugar se escogieron los paquetes **caret** [@caret] y **xgboost** [@xgboost] para construir el modelo; y el paquete **forecast** [@forecast] para visualizar sus predicciones. El modelo se construyó en base a las instrucciones brindadas de este en un blog [@articuloxgboost]. El problema de esta opción fue que el modelo creado no reconocía que la variable provista era el tiempo. Como indica la guía, la variable de tipo *Date* utilizada como explicativa se separa en dos: una indica el año y la otra el mes de la fecha correspondiente. Sin embargo, para el modelo esas dos variables son simplemente numéricas, y lo que es peor, son tratadas como independientes entre sí. 

Este problema se reflejó en predicciones que mostraban un patrón de variaciones que se repetía año a año, pero a pesar de la clara tendencia de aumento de los precios a lo largo del tiempo, los predichos se mantenían en un mismo rango. A continuación se muestra el resultado de las predicciones utilizando esta herramienta.


```{r, warning = F, fig.cap = "Prediccion del precio promedio de los fideos. El tramo en azul corresponde al período predicho.",message=F}
fideos <- precios %>% 
...
productos2 <- levels(as.factor(precios$producto))
...
server <- function(input, output) {
   observe({
    updateSelectInput(session=getDefaultReactiveDomain(),"eleg_marca","Marca:",choices=levels(as.factor(filter(preciosmvd,producto %in% input$eleg_producto)$marca)))
  ...
  prod_dpto <- precios %>% 
  filter(id_producto %in% input$eleg_producto, marca %in% input$eleg_marca) %>% 
  group_by(id.depto) %>% 
  summarise(precio = mean(avg))
  output
...
Un análisis a futuro podría incluir distintos productos y la utilización de ponderadores para generar una canasta básica, como los que generó la escuela de nutrición [@canastanutricion]. De esa forma sería posible analizar los productos de manera agregada en vez de individualmente.
