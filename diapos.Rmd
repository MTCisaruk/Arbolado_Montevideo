...
establecimientos <- read_delim("datos/establecimiento.csv", delim = ";") |> mutate(lat=as.numeric(sub(',','.',lat,fixed=T)),long=as.numeric(sub(',','.',long,fixed=T)))
df_depto <- st_as_sf(vect(here("Mapas","ine_depto.shp")))
df_mvdeo <- fortify(readOGR(here("Mapas","ine_ccz_mvd.shp")))
```

```{r mapa uruguay}
establecimientosCord <- filter(establecimientos,!(is.na(lat) | is.na(long)))
establecimientosCord <- st_as_sf(establecimientosCord,coords = c("lat", "long"))
ggplot() + geom_sf(data=df_depto,fill=NA,color="black") +
