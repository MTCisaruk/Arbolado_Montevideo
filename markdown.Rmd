...
productos <- read_csv2("datos/productos.csv",locale = readr::locale(encoding = "latin1")) %>% filter(id.producto %in% c(1,2,3,13,14,15,16,17,18,19,20,21,23,22,24,25,29,30,40,41,48,49,50,52,53,54,61,62,76,77,78,85,86,87,102,103,104,121,122,123,124,130,131,132,133,134,135,140,141,142,149,150,151,26,27,28,55,56,57,359,361,365))
```
...
HAY QUE AGREGAR LOS PRODUCTOS 359,361 Y 365 Y QUITAR 74 Y 75
...
```{r}
precios2 <- mutate(precios,year_month=as.Date(paste(str_sub(year_month,start=1,end=4),str_sub(year_month,start=5,end=6),sep="-")))
ggplot(filter(precios,id_producto %in% 55:57),aes(x=year_month,y=avg)) + geom_point()
```
